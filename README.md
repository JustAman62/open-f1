...
| `enablePositionSmoothing`                     | N/A                         | `UNDERCUTF1_ENABLEPOSITIONSMOOTHING`                      | Whether car positions on the driver tracker glide between updates by fitting recent samples (the dot lags live slightly), rather than jumping to each raw sample. Default: `true`. Values: `true` or `false`. |
